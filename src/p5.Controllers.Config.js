p5.prototype.ChineseCharacter.prototype.defaultConfig = {
	...
	TRIM_STRATEGIES: {
		...
		'linear': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * angle,
		// 'sin_cos': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * (Math.sin(angle) + Math.cos(angle)) / 2,
		'sin_cos': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * (Math.sin(amplitude * dynamicTrimRatio) + Math.cos(amplitude * dynamicTrimRatio)) / 2,
		// 'sin': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.sin(angle * amplitude * dynamicTrimRatio),
		'sin': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.sin(amplitude * dynamicTrimRatio),
		// 'cos': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.cos(angle),
		'cos': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.cos(amplitude * dynamicTrimRatio),
		// 'cos_2': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * (Math.cos(angle) / 2),
		'cos_2': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * (Math.cos(amplitude * dynamicTrimRatio) / 2),
		// 'tan': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.tan(angle),
		'tan': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.tan(amplitude * dynamicTrimRatio),
		'power': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.pow(angle, 0.1),
		'exp_log': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * (Math.exp(angle) + Math.log(Math.max(Math.abs(angle), 1e-6))) / 2,
		'exp': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.exp(angle),
		'log': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * Math.log(Math.max(Math.abs(angle), 1e-6)),
		'log_2': (angle, max_trim_ratio, amplitude, dynamicTrimRatio) => max_trim_ratio * (Math.log(Math.max(Math.abs(angle), 1e-6)) / 2),
	}
};
